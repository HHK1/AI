...
Even though this discovery has had some great echoes in the scientific
community, it remains anecdotic. In order to create this flaw, a random noise
was added to a correctly classified image, \textit{until} it was not classified
correctly. But for a vast majority of cases, the classification was not altered
by the noise. Moreover, in real life applications, let's assume in an autonomous
vehicle: if a subject was not correctly interpreted due to  some random noise,
it would certainly be correctly interpreted during a second pass in the system
(in a video feed for example), or it could be caught and treated as an error by
the system itself.\\
